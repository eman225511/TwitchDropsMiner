...
echo [1/8] Getting git information...
...
echo.
echo [2/8] Appending git revision to version.py...
...
echo [4/8] Building Windows executable with PyInstaller...ul
...
echo [5/8] Creating Windows release package...
...
echo [6/8] Building Linux PyInstaller (x86_64)...
...
gh release create dev-build ^
    ...
    --notes "**This is an automatically generated in-development pre-release version of the application, that includes the latest master branch changes.**%NL%**⚠️ This build is not stable and may end up terminating with a fatal error. ⚠️**%NL%**Use at your own risk.**%NL%%NL%- Last build date: `%DATE_NOW%`%NL%- Reference commit: %SHA_FULL%"
